...
{
public:
    string reg_str[32] = {"x0", "a0", "a1", "a2", "a3","a4", "a5", "a6", "a7" ,"t0", "t1", "t2", "t3", "t4", "t5", "t6", "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7", "s8", "s9","s10", "s11","ra", "tp", "gp", "sp"};
...
public:
    ...
    void print_userdata(map<int, int> &data, int b, int e)
    {
        ...
        {
            cout << "<Memory> "
                 << "0x" << setfill('0') << setw(8) << hex << i << " : " << dec << data[i] << endl;
        }
    }
    void print_data(map<int, int> &data, int bgp, int bst, int egp, int est)
    {
        ...
        {
            cout << "<Memory> "
                 << "0x" << setfill('0') << setw(8) << hex << i << " : " << dec << data[i] << "            |          "
                 << "<Memory> "
                 << "0x" << setfill('0') << setw(8) << hex << j << " : " << dec << data[j] << endl;
        }
    }
};
class parser
...
public:
    ...
    void execute(vector<string> &code, map<string, int> &reg_map, int *regs, map<string, int> &data_r, map<int, int> &data, map<string, int> &branches)
    {
        ...
        for (int i = 0; i < code.size(); ++i)
        {
            if (code[i] == "")
                continue;
            else if (regex_match(code[i], sm, br_empty_e))
                continue;
            else if (regex_match(code[i], sm, r_format_e))
            {
                ...
                else
                {
                    cout << "Error at line " << i << " : "
                         << "invalid instruction R ADDRESS" << endl;
                    break;
                }
                regs[0] = 0;
            }
            else if (regex_match(code[i], sm, i_format_e))
            {
                ...
                else
                {
                    ...
                    cout << "Error at line " << i  << " : "
                         << "invalid instruction I ADDRESS" << endl;
                    break;
                }
            }
            ...
            else if (regex_match(code[i], sm, load_e))
            {
                ...
                {
                    cout << "Error at line " << i  << " : "
                         << "invalid instruction SW ,LW" << endl;
                    break;
                }
                if (sm[2] == "lw")
                {
                ...
                {
                    cout << "Error at line " << i  << " : "
                         << "invalid instruction LW,SW ADDRESS" << endl;
                    break;
                }
            }
            else if (regex_match(code[i], sm, cond_branch_e))
            {
                ...
                {
                    cout << "Error at line " << i << " : "
                         << "invalid instruction beq" << endl;
                    break;
                }
                if (br == 0)
                {
                    cout << "Error at line " << i  << " : "
                         << "invalid instruction bne" << endl;
                    break;
                }
                ...
                else
                {
                    cout << sm[2] << endl;
                    cout << "Error at line " << i  << " : "
                         << "invalid instruction branch end " << endl;
                    break;
                }
            }
...
class RISC_Vsimulator : screen, registers, parser, execution
{
public:
    string input_file;
 
    RISC_Vsimulator(string input_file)
    {
...
int main()
{
    ...
    cin >> file;
    RISC_Vsimulator *mp = new RISC_Vsimulator(file);
    mp->run();
}